...
int main(void) {
    ...
    const double Fcont = 2000.0; // "continuous" grid for plotting
    const double Fe    = 40.0;   // sampling frequency
    const int    Nbits = 4;      // quantization bits
    const double Tshow = 2.0 / f1; // 2 periods

    const double Amax = fabs(A1);
    ...
    for (int n = 0; n < Ncont; n++) {
        double t = n / Fcont;
        double x = x_signal(t, A1, f1, phi1);
    ...
    for (int n = 0; n < Ns; n++) {
        t_s[n] = n / Fe;
        x_s[n] = x_signal(t_s[n], A1, f1, phi1);
    ...
    // ---------------- MMSE (LMMSE) estimation after quantization ----------------
    // x_hat[n] = alpha * x_q[n]
    ...
    // ---------------- Summary ----------------
    printf("----- Summary -----\n");
    printf("Eq1: x(t)=A1*sin(2*pi*f1*t+phi1)\n");
    printf("A1=%.3f, f1=%.3f Hz, phi1=%.3f rad\n", A1, f1, phi1);
