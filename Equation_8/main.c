...
// ---------------- Equation 8 ----------------
// x(t) = A*sin(2*pi*f1*t) + B*sin(2*pi*f2*t)
static double x_signal(double t, double A, double f1, double B, double f2) {
    return A * sin(2.0 * M_PI * f1 * t)
         + B * sin(2.0 * M_PI * f2 * t);
}
...
int main(void) {
    // ---------------- Parameters (consistent with Eq3/Eq6/Eq7 style) ----------------
    const double A  = 1.0;
    ...
    for (int n = 0; n < Ns; n++) {
        t_s[n] = n / Fe;
        x_s[n] = x_signal(t_s[n], A, f1, B, f2);
    ...
    // ---------------- Summary ----------------
    printf("----- Summary -----\n");
    printf("Eq8: x(t)=A*sin(2*pi*f1*t) + B*sin(2*pi*f2*t)\n");
    printf("A=%.3f f1=%.3fHz | B=%.3f f2=%.3fHz\n", A, f1, B, f2);
