...
// ---------------- Equation 6 ----------------
// x(t) = A1*sin(2*pi*f1*t) + B1*cos(2*pi*f2*t + phi2)
static double x_signal(double t, double A1, double f1,
                       double B1, double f2, double phi2) {
    return A1 * sin(2.0 * M_PI * f1 * t)
         + B1 * cos(2.0 * M_PI * f2 * t + phi2);
}
...
int main(void) {
    // ---------------- Parameters (keep consistent with your previous ones) ----------------
    const double A1 = 1.0;
    ...
    const double Fcont = 4000.0; // smoother for sum signals
    const double Fe    = 80.0;   // must be > 2*max(f1,f2) => >16 (80 is safe)
    const int    Nbits = 4;

    // keep your classic view window
    const double Tshow = 0.4;

    // Safe saturation range: worst-case sum of amplitudes
    const double Amax = fabs(A1) + fabs(B1);
    ...
    for (int n = 0; n < Ncont; n++) {
        double t = n / Fcont;
        double x = x_signal(t, A1, f1, B1, f2, phi2);
    ...
    for (int n = 0; n < Ns; n++) {
        t_s[n] = n / Fe;
        x_s[n] = x_signal(t_s[n], A1, f1, B1, f2, phi2);
    ...
    // ---------------- Summary ----------------
    printf("----- Summary -----\n");
    printf("Eq6: x(t)=A1*sin(2*pi*f1*t) + B1*cos(2*pi*f2*t+phi2)\n");
    printf("A1=%.3f f1=%.3fHz | B1=%.3f f2=%.3fHz phi2=%.3f\n", A1, f1, B1, f2, phi2);
