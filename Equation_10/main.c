...
static double x_signal(double t, double A1, double f1, double A2, double f2) {
...
int main(void) {
    // ---------------- Parameters (same style as Eq3/Eq6/Eq7/Eq8/Eq9) ----------------
    const double A1 = 1.0;
    ...
    for (int n = 0; n < Ncont; n++) {
        double t = n / Fcont;
        double x = x_signal(t, A1, f1, A2, f2);
    ...
    for (int n = 0; n < Ns; n++) {
        t_s[n] = n / Fe;
        x_s[n] = x_signal(t_s[n], A1, f1, A2, f2);
    ...
    // ---------------- Summary ----------------
    printf("----- Summary -----\n");
    printf("Eq10: x(t)=A1*sin(2*pi*f1*t) + A2*cos(2*pi*f2*t)\n");
    printf("A1=%.3f f1=%.3fHz | A2=%.3f f2=%.3fHz\n", A1, f1, A2, f2);
