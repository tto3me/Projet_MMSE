...
// ---------------- Equation 9 ----------------
// x(t) = A*cos(2*pi*f1*t + phi1) - B*cos(2*pi*f2*t)
static double x_signal(double t, double A, double f1, double phi1,
                       double B, double f2) {
    return A * cos(2.0 * M_PI * f1 * t + phi1)
         - B * cos(2.0 * M_PI * f2 * t);
}
...
int main(void) {
    // ---------------- Parameters (consistent with Eq6-8 style) ----------------
    const double A    = 1.0;
    ...
    const double Amax = fabs(A) + fabs(B);
    ...
    for (int n = 0; n < Ncont; n++) {
        double t = n / Fcont;
        double x = x_signal(t, A, f1, phi1, B, f2);
    ...
    for (int n = 0; n < Ns; n++) {
        t_s[n] = n / Fe;
        x_s[n] = x_signal(t_s[n], A, f1, phi1, B, f2);
    ...
    // ---------------- Summary ----------------
    printf("----- Summary -----\n");
    printf("Eq9: x(t)=A*cos(2*pi*f1*t+phi1) - B*cos(2*pi*f2*t)\n");
    printf("A=%.3f f1=%.3fHz phi1=%.3f | B=%.3f f2=%.3fHz\n", A, f1, phi1, B, f2);
